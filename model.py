...
def Generator():
    ...
    x = Conv2D(32, 3, strides=2, activation=act, padding='same')(inputs)
    x = Conv2D(64, 3, activation=act, padding='same')(x)
    sum1 = Conv2D(128, 1, strides=2, padding='same')(x)
    x = SeparableConv2D(128, 3, padding='same', activation=act)(x)
    ...
    x = Activation(act)(x)
    x = SeparableConv2D(256, 3, padding='same', activation=act)(x)
    x = SeparableConv2D(256, 3, padding='same')(x)
    x = MaxPooling2D((3, 3), strides=(2, 2), padding='same')(x)
    x = add([x, sum2])

    sum3 = Conv2D(728, 1, strides=2, padding='same')(x)
    x = Activation(act)(x)
    x = SeparableConv2D(728, 3, padding='same', activation=act)(x)
    ...
    sum5 = Conv2D(1024, 1, strides=2, padding='same')(x)
    x = Activation(act)(x)
    x = SeparableConv2D(728, 3, padding='same', activation=act)(x)
    x = SeparableConv2D(1024, 3, padding='same')(x)
    x = MaxPooling2D((3, 3), strides=(2, 2), padding='same')(x)
    x = add([x, sum5])
    x = SeparableConv2D(1536, 3, padding='same', activation=act)(x)
    x = SeparableConv2D(2048, 3, padding='same', activation=act)(x)
    ...
    x = Dense(196608, activation='sigmoid')(x)
    predictions = Reshape((256, 256, 3))(x)
